...
(function () {
  ...
  function blockSignature(block) {
    if (block.kind === "packetContext" && block.sections) {
      return block.sections
        .map(function (section) {
          return section.name + "\x00" + (section.body || "");
        })
        .join("\n");
    }
  ...
  function appendSectionProse(parent, text) {
    var line = (text || "").trim();
    if (!line) return;
    var pre = document.createElement("pre");
    pre.className = "cgw-continuous-packet-context__pre";
  ...
  function appendRichBlock(parent, block) {
    ...
    if (block.kind === "packetContext") {
      var panel = document.createElement("div");
      panel.className = "cgw-continuous-packet-context cgw-continuous-block";
      panel.setAttribute("data-cgw-packet-context", "1");

      var sectionCount =
        block.sectionCount || (block.sections && block.sections.length) || 0;

      var details = document.createElement("details");
      details.className = "cgw-continuous-packet-context__details";

      var turnId =
        parent && parent.getAttribute
          ? parent.getAttribute("data-cgw-turn-id")
          : null;
      var expandState = globalThis.__cgwPacketContextExpandState;
      if (turnId && expandState && expandState.get(turnId)) {
        details.open = true;
      }
      details.addEventListener("toggle", function () {
        if (turnId && expandState) expandState.set(turnId, details.open);
      });
      ...
      sectionsWrap.className = "cgw-continuous-packet-context__sections";

      (block.sections || []).forEach(function (section) {
        var sec = document.createElement("section");
        sec.className = "cgw-continuous-packet-context__section";

        var heading = document.createElement("div");
        heading.className = "cgw-continuous-packet-context__heading";
        heading.textContent = "[" + section.name + "]";

        var bodyWrap = document.createElement("div");
        bodyWrap.className = "cgw-continuous-packet-context__body";
        appendSectionProse(bodyWrap, section.body || "");
      ...
      details.appendChild(sectionsWrap);
