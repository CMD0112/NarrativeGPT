...
internal static class DiagnosticsLog
{
    ...
    public static string UnifiedTracePath => Path.Combine(DiagnosticsPaths.ConfigRoot, "wrapper-diagnostics.jsonl");

    public static string PlaySendLegacyPath => Path.Combine(DiagnosticsPaths.ConfigRoot, "play-send-trace.jsonl");
    ...
    public static void LogFromPage(JsonElement root, bool mirrorPlaySendLegacy = false)
    {
        ...
        var run = DiagnosticsPlaySendContext.GetActiveRun?.Invoke();
        Write(
            channel,
            level,
            eventName,
