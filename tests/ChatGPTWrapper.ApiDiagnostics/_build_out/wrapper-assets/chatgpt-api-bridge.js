(function () {
  ...
  function reply(id, payload) {
    post(Object.assign({ channel: BRIDGE_CHANNEL, id: id }, payload));
  }
  ...
  async function downloadFile(cmd) {
    ...
    var paths = [
      "/backend-api/files/" + encodeURIComponent(fileId) + "?download=1",
    ...
    for (var i = 0; i < paths.length; i++) {
      try {
        ...
        if (!res.ok) {
          lastErr = { status: res.status, path: paths[i] };
          continue;
        }
        ...
        lastErr = { message: e && e.message ? String(e.message) : "unknown" };
      }
    }
  ...
  ensureMessageListener();
  post({ channel: BRIDGE_CHANNEL, type: "apiBridgeReady", ok: true });
})();
