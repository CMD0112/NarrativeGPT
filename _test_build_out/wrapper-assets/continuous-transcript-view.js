...
(function () {
  ...
  function extractTurnBlocks(turn, turnId, role) {
    var tid = String(turnId);
    var nativeFp = computeNativeTurnFingerprint(turn);
    var cached = turnExtractCache[tid];
    var rawBlocks;

    if (cached && cached.nativeFp === nativeFp && cached.rawBlocks) {
      rawBlocks = cached.rawBlocks;
    } else {
      rawBlocks = formatTurnToBlocks(turn);
      turnExtractCache[tid] = {
        nativeFp: nativeFp,
        rawBlocks: rawBlocks,
        role: role,
      };
  ...
  function updateSegmentBlocksIncremental(segEl, blocks, prevBlockFps, turnId) {
    if (
      prevBlockFps &&
      blocks.length > 0 &&
  ...
  function syncOverlayGeometry(scrollHost, container) {
    if (!scrollHost || !container) return;
    updateComposerClearance();
    var h = scrollHost.clientHeight;
    if (h > 0) {
  ...
  function applyContinuousView() {
    ...
    trimTurnExtractCache(segments.map(function (s) {
