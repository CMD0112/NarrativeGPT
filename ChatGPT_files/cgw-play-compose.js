(function () {
  ...
  function syncNativeComposerSendLock() {
    if (globalThis.__cgwWrapperComposer) return;
    var state = globalThis.__cgwPlayComposeState || {};
    var locked = !!(state.busy || sendInFlight || globalThis.__cgwComposeSendInFlight);
    var root = document.documentElement;
    if (!root) return;
    if (locked) root.setAttribute("data-cgw-native-send-locked", "1");
    else root.removeAttribute("data-cgw-native-send-locked");
  }
