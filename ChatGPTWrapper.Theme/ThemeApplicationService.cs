...
public static class ThemeApplicationService
{
    ...
    /// <summary>Clears the WPF apply cache so the next <see cref="ApplyToWpf"/> runs on a fresh Application (e.g. WinUI STA host).</summary>
    public static void ResetWpfApplyFingerprint() => _lastAppliedFingerprint = null;

    private static string? _lastAppliedWinUiFingerprint;
    private static Action<ResolvedTheme>? _winUiApplyHandler;

    public static void RegisterWinUiApplyHandler(Action<ResolvedTheme> handler) =>
        _winUiApplyHandler = handler;

    public static bool ApplyToWinUi(ResolvedTheme theme)
    {
        ArgumentNullException.ThrowIfNull(theme);

        var fingerprint = theme.ComputeFingerprint();
        if (string.Equals(_lastAppliedWinUiFingerprint, fingerprint, StringComparison.Ordinal))
            return false;

        _winUiApplyHandler?.Invoke(theme);
        _lastAppliedWinUiFingerprint = fingerprint;
        return true;
    }
    ...
    public static void InvalidateApplyCache()
    {
        ...
        _lastAppliedWinUiFingerprint = null;
