(function () {
  ...
  function waitForComposer(timeoutMs, onReady) {
    var deadline = Date.now() + (timeoutMs || 5000);
    function poll() {
      var probe = probeComposer();
      if (probe.composerFound && probe.submitFound) {
        onReady(probe);
        return;
      }
      if (Date.now() >= deadline) {
  ...
  function sendPrompt(text, timeoutMs, requireProjectContext) {
    var restoreWrapper = temporarilyShowNativeComposer();
  ...
  function handleCommand(cmd) {
    ...
    switch (cmd.action) {
      case "sendPrompt":
        ...
        break;
      case "fillComposer": {
        var restoreFill = temporarilyShowNativeComposer();
        setTimeout(function () {
          waitForComposer(5000, function (probe) {
