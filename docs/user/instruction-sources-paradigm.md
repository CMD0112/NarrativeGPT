...
Packet profiles are documented in [adventure-developer-reference.md §5](../developer/adventure-developer-reference.md#5-prompt-packets-three-profiles). Inline fallback inlines static lore when `ForceInlineLore` is on or the user proceeds after the publish warning.
...
**Delivery orchestration:** Utility jobs use a readiness gate and tiered send (API vs atomic DOM). See [utility-job-orchestration.md](../developer/utility-job-orchestration.md).
...
| Prompt audit | No | `prompt-history.json` (v2 flight records — [data-model-reference.md](../reference/data-model-reference.md#prompthistorydocument)) |
