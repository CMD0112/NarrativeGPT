...
User-facing reference for the **Adventure panel** in ChatGPT Wrapper: UI surfaces, dialogs, workflows, and smoke checklists. For data model, turn automation, and service internals, see [Adventure Developer Reference](../developer/adventure-developer-reference.md).

**Documentation hub:** [INDEX.md](../INDEX.md)

**Related docs:** [Adventure Developer Reference](../developer/adventure-developer-reference.md) · [Projects & Source Sync (user guide)](user-projects-and-sync.md) · [Instruction Contract Guide](instruction-contract-guide.md) · [Data Model Reference](../reference/data-model-reference.md) · [Services Reference](../reference/services-reference.md) · [Instruction vs Sources Paradigm](instruction-sources-paradigm.md) · [WebView Bridges](../developer/webview-bridges.md) · [Troubleshooting](troubleshooting.md) · [Play surface UX modernization ADR](../adr/play-surface-ux-modernization-adr.md) (CMD-415) · [Implementation plan](../plans/play-surface-ux-modernization-implementation-plan.md)
...
Play and Design are entered from the dashboard (not toolbar mode buttons). **Play ↔ Design** in-session: shell **Play / Design** `SegmentedControl` (same adventure session), Play **Continue design…**, and Design **Launch adventure** — see [Play/Design surface convergence ADR](../adr/play-design-surface-convergence-adr.md).

### Unified session chrome (Play / Design)

When an adventure session is active, the shell context row owns session IA (no duplicate play header buttons):
...
Implementation: `MainWindow.SessionChrome.cs`, `PlayLayoutCapabilities.UseUnifiedSessionChrome`.
...
**Layout customization:** Play settings → **Play surface** offers named presets (Writer, GM, Minimal), per-tab placement (`Reference`, `Warnings`, `State`, `Notes` → Left / Right / Hidden), and **cockpit on enter** preferences (global): remember last tab/section, always collapsed, or always open; default companion tab and cockpit section on first visit. Per-adventure persistence: `PlayCompanionLastTab`, `PlayCompanionLastSection` (when **Remember cockpit section** is enabled). `PlayPanelLayoutService` and `PlayLayoutPresetLibrary` apply placement; `NavigateToPlayTab` expands the correct column and selects the tab in either host.
...
Removed — session chrome is owned by the application shell (back, title, Play | Design segment, status chips, session overflow). Legacy header handlers remain in code-behind for shell bridges only.

### Session cockpit

Top card in the play side panel with a **Session | Narrator | Tools** `SegmentedControl` (one section visible at a time — no nested expanders).
...
Pending-review shortcuts appear in the Reference tab review queue banner when navigated from Entities / `FocusEntityReviewQueue`.
...
`ShellCommandBarStyle` row: **Search**, **Export**, and **More** menu for **Recap** (local digest, non-AI), Branch, Roll, Move to new play thread, Continue design. Use dashboard **Backup** or **Export…** (ZIP) for snapshots — not in-play Save state. Icon-only mode below 280px width.
...
### Flight recorder (Play settings → History)

Read-only audit of **what the model received** on each verified send. Distinct from **Next send** preview (draft) and from `log.json` (accepted transcript).

| Check | Expected |
|-------|----------|
| After 3+ Sends | History timeline lists entries with turn #, time, player line snippet, mode, verified ✓ |
| Select a row | Injection sections grid matches **Next send** manifest taxonomy; baseline + THIS TURN pointer lists |
| Bundled utility | **Bundled utility runs** shows job label, lane, manifest summary when injection-first jobs were in the packet |
| Send trace | Short run id + **Copy run id**; optional event excerpt from `play-send-runs/{id}.json` |
| Changes vs prior | Summary line; section **Added** / **Removed** / **Trimmed** badges; **NEW** on new THIS TURN pointers |
| Compare packet | Unified line diff dialog vs previous send (or pick another baseline in **Compare to**) |
| View / copy | **View full…** and **Copy packet** work on delivered text |
| Long session | 20+ entries scroll without UI freeze (virtualized list) |
| On disk | `%LocalAppData%\ChatGPTWrapper\adventures\{id}\prompt-history.json` has `schemaVersion: 2` |

**Troubleshooting:** [troubleshooting.md — Inspect a flight record](troubleshooting.md#inspect-a-flight-record-for-a-turn). **Schema:** [data-model-reference.md — PromptHistoryDocument](../reference/data-model-reference.md#prompthistorydocument).

**Canonical play record:** The linked ChatGPT play thread is the narrative source of truth. Per-thread logs live in `thread-logs/{threadEntryId}/rolling.jsonl` (append-only, includes superseded edits). Rolling sync runs after send, edits, and session load. Use **Dump thread log** in More actions for a full API snapshot. See [thread-conversation-log-adr.md](../adr/thread-conversation-log-adr.md).
...
- **History** — **Flight recorder** (SVA-03): per-turn audit of verified sends — timeline, injection manifest, pointers, utility/trace correlation, packet diff vs prior send ([ADR](../adr/narrative-flight-recorder-adr.md))
...
Full pipeline reference: **[utility-job-orchestration.md](../developer/utility-job-orchestration.md)**
...
### Thread conversation log

Rolling JSONL per registry thread (`thread-logs/{threadEntryId}/`). **Sync thread log** reconciles from the live API; **Dump thread log** writes a full conversation snapshot to `dumps/`.
...
|---------|----------|---------------|
| Packet profiles | [Instruction vs Sources](instruction-sources-paradigm.md) | [§5 Prompt packets](../developer/adventure-developer-reference.md#5-prompt-packets-three-profiles) |
| Project linking | [Projects & Source Sync](user-projects-and-sync.md) | [§6 Project linking](../developer/adventure-developer-reference.md#6-chatgpt-project-linking) |
| Source publish/sync | [Projects & Source Sync](user-projects-and-sync.md) | [§7 Source export and sync](../developer/adventure-developer-reference.md#7-source-export-and-sync) |
| Packet construction | [Prompt Construction Guide](prompt-construction-guide.md) | [adventure-developer-reference.md](../developer/adventure-developer-reference.md) |
...
**Data model and persistence:** JSON documents under `%LocalAppData%\ChatGPTWrapper\adventures\{guid}\`. See [Data Model Reference](../reference/data-model-reference.md) and [adventure-developer-reference.md §2–3](../developer/adventure-developer-reference.md#2-data-model).
...
- Packet preview: `[[cgw:sources]]` → **ALWAYS RETRIEVE** (see [[adventure-developer-reference.md §5](../developer/adventure-developer-reference.md#5-prompt-packets-source-delegated-vs-fat-fallback))
