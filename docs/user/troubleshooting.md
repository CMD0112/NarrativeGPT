...
## Inspect a flight record for a turn

**Symptoms:** “What did the model see?” — wrong lore, missing section, trim surprise, utility context doubt.

**UI path:**

1. **Play settings → History** (flight recorder).
2. Select the turn in the timeline (or use **Compare to** for a different baseline).
3. Review **Injection sections**, **This turn pointers**, **Bundled utility runs**, and **Changes vs prior send**.
4. **Compare packet…** for a unified line diff vs the prior verified send.
5. **Copy run id** → open `%LocalAppData%\ChatGPTWrapper\play-send-runs\{shortId}.json` for the full send timeline.

**On-disk audit:**

```
%LocalAppData%\ChatGPTWrapper\adventures\{adventure-guid}\prompt-history.json
```

Find the entry whose `turnId` matches the turn in `log.json`, or match by `at` / `playerLine`. v2 entries include `injection`, `delivery`, `utilityRuns`, and `playSendTraceRunId`.

**Cross-checks:**

| Question | Where to look |
|----------|----------------|
| Did preview match send? | Compare **Next send** manifest to History row for the same send — same section taxonomy ([CMD-295](https://linear.app/cmd0112/issue/CMD-295)) |
| Why was a section omitted? | History → section row note / **Trimmed** badge; `injection.trimmed` in JSON |
| Why did THIS TURN include a pointer? | Pointer `source` label (Name match, State, Pin, …) |
| Utility job context | **Bundled utility runs** manifest; `utility-results/{runId}.json` |
| Send pipeline failure | `play-send-trace.jsonl` + run summary JSON linked from trace run id |

**Docs:** [narrative-flight-recorder-adr.md](../adr/narrative-flight-recorder-adr.md) · [data-model-reference.md — PromptHistoryDocument](../reference/data-model-reference.md#prompthistorydocument)
...
5. Run live API diagnostics (developers): see [Testing — Live diagnostics](../developer/testing.md#live-diagnostics).
...
- Full error reference: [utility-job-orchestration.md § Error codes](../developer/utility-job-orchestration.md#error-codes)
...
See [Utility Job Orchestration](../developer/utility-job-orchestration.md) for the full pipeline.
...
See [Testing](../developer/testing.md) for full checklist steps and env vars.
...
- [WebView Bridges](../developer/webview-bridges.md) — bridge command failures
- [ChatGPT API Integration](../developer/chatgpt-api-integration.md) — endpoint errors
- [Testing](../developer/testing.md) — automated diagnostics
