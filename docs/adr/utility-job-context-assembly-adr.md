...
**E2E review:** [Enhancements/utility-job-e2e-review.md](../Enhancements/utility-job-e2e-review.md)
...
### Operational prerequisites (field)

Context assembly runs only after jobs are routed and pushed. Extended diagnostics (2026-06) showed:

- **Worker pin** (`HasWorkerPin`) must be present — green `UtilityWorkerCapabilities` alone is insufficient. `TryReconcilePinFromCapabilities` restores pin from caps when stale saves dropped registry binding.
- **API `http_403`** is common; worker lane uses DOM/off-screen send. Packets are often 6–8k chars — dedup (CMD-393, CMD-396) matters for latency and model focus.
- **`WorkerOnly` policy** routes all utility jobs to worker outbox — worker assembler path is the primary production surface.

See [utility-job-e2e-review.md](../Enhancements/utility-job-e2e-review.md).
...
## 9. Migration / rollout (v1 landed 2026-06-28)

1. **CMD-392** — Assembler behind `UseUtilityJobContextAssembler` (default `true`); worker, play bundled, utility-only, legacy inline.
2. **CMD-393** — Bundled dedup with `PlayPacketContextSnapshot` from play send.
3. **CMD-394–396** — Worker lore channel, lexical canon slices, handler dedup (`UtilityStoryContextDedup`).
4. **CMD-397** — Preview manifest + persist on `UtilityJobRunRecord`.
5. **CMD-398** — Docs + tracker sync.

**Still open:** Field QA on 50+ play turns; v2 improvements tracked in [CMD-401](https://linear.app/cmd0112/issue/CMD-401) (legacy retirement, adaptive budgets, preview parity, outcome correlation); semantic synergy in [CMD-399](https://linear.app/cmd0112/issue/CMD-399) (CMD-401 workstream A).
...
- [ ] Worker `continuity_check` useful after 50+ play turns with zero worker history *(manual QA)*
- [x] Injection-first bundled jobs do not duplicate summary/transcript in same send
- [x] AI Actions preview shows lane + manifest
- [x] One assembler replaces three divergent call sites (when flag enabled)
- [x] Linked Project: worker jobs include lore retrieval guidance without inlining full sources

---

## Key code references (v1)

| File | Role |
|------|------|
| `UtilityJobContextAssembler.cs` | Single entry — lanes, lore prepend, manifest |
| `UtilityJobContextPreviewService.cs` | Preview via assembler |
| `PlayPacketContextSnapshotBuilder.cs` | Bundled overlap snapshot |
| `UtilityStoryContextDedup.cs` | Job-core dedup vs story block |
| `UtilityWorkerLoreChannelService.cs` | Worker sources block orchestration |
| `UtilityCanonSliceSelector.cs` | Lexical slice selection + inline caps |
| `UtilityCanonSliceProfiles.cs` | Per-job excerpt budgets |
| `UtilityJobScopeSignals.cs` | Scope signals for canon/lore |
| `UtilityContextManifest.cs` / `UtilityContextManifestRecord.cs` | Manifest + persistence |
| `ContextPointerResolver.ResolveTaskScoped` | Task-scoped pointers (no narrator baseline) |
| `ContextPointerRenderer.BuildUtilityWorkerSourcesBlock` | `mode="utility-worker"` + INLINE EXCERPTS |
| `UtilityStoryContextBuilder.cs` | Story block sections |
| `PlayUtilityInjectionService.cs` | Injection-first sections |
| `UtilityWorkerOrchestrator.cs` | Worker orchestration |
| `GenerationJobService.cs` | Legacy inline assembly |
| `GenerationJobHandlers.cs` | Job body (deduped slices) |
...
| [CMD-401](https://linear.app/cmd0112/issue/CMD-401) | v2 icebox umbrella (adaptive assembly, quality, legacy) |
