...
2. **Local canonical log:** `thread-logs/{threadEntryId}/rolling.jsonl` per registry thread (Play, Design, UtilityWorker), synced from the live API branch after send, edits, and session load.
3. **Legacy retirement:** `log.json` and `thread-metadata.json` are migrated on load and no longer written on send.
