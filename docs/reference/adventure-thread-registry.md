...
**Threads hub** (`AdventureThreadManagerDialog`) is the primary surface for project link, play/design pins, handoff, and inline delivery toggles. Authors can **create thread slots**, **pin browser tabs to a selected row** (current tab or picker), **clear pins**, **archive** inactive threads, and **remove** archived rows from the list. Play and design cockpits show a compact **Connection** line (`FormatConnectionSummary`) that opens the hub.
...
| `RemoveEntry` | Permanently remove archived row from registry |
| `ClearEntryPin` | Clear pin triple on an entry |
| `EntryHasPin` | Whether entry has tab or URL pin |
