...
### Flight recorder (SVA-03)

| Service | Role |
|---------|------|
| `FlightRecordCaptureService` | Build v2 `prompt-history.json` entry from `PreparedSendArtifact` + delivery snapshot at verified send |
| `FlightRecordCorrelationService` | Utility run rows, log-turn link, trace excerpt from `play-send-runs/` |
| `FlightRecordCompareService` | Manifest/pointer delta vs baseline; packet unified diff via `TextDiffService` |
| `FlightRecordDetailFormatter` | Timeline/detail labels for `FlightRecorderPanel` |
| `PromptHistoryMigration` | v1 → schema 2 on `AdventureStore.Load` |

**UI:** `Views/FlightRecorderPanel` — Play settings → **History** tab.

**Called by:** `PlaySendOrchestrator` (capture), `PlayPromptInjectionDialog.BindHistory`
...
> **Full pipeline doc:** [utility-job-orchestration.md](../developer/utility-job-orchestration.md) — readiness gate, atomic DOM turns, session reuse, error codes.

### EphemeralProjectChatService

**Role:** One-shot linked-project chat without adventure thread binding.
...
Central auto-sync after entity edit or one-click repair. **TrySyncAfterEntityEdit** builds an **EntityChangePlan** (via **EntityChangePlanBuilder**), applies export + cross-canon text + rename reconciliation, sets notify flags, and returns **RequiresManualReconcile** when drift remains. **RepairFromJson** re-runs sync from current JSON (session status click). See [entity-canon-change-paradigm.md](../user/entity-canon-change-paradigm.md).
...
Structured narrator contract: global boundaries, character portrayal rules, addendum. Builds contract sections for instructions and packets; hydrates Design → Instructions fields; parses `instructions-snippet.md` on import. See [instruction-contract-guide.md](../user/instruction-contract-guide.md).
...
- [ChatGPT API Integration](../developer/chatgpt-api-integration.md)
- [WebView Bridges](../developer/webview-bridges.md)
- [Adventure Developer Reference §9](../developer/adventure-developer-reference.md#9-key-play-path-services)
