...
4. If the user approves, update **team workflow settings**, then sync **both** canons (`docs/linear-issue-reference.md` + Linear taxonomy doc) and `linear-integration.md` if git automations apply.
...
`docs/linear-issue-reference.md` and the [Issue Taxonomy & Workflow Guide](https://linear.app/cmd0112/document/issue-taxonomy-and-workflow-guide-1d30b366d19d) are **dual canons**. Any taxonomy or workflow change must be reflected in **both** in the same agent session — never update only one.
...
1. Open `docs/linear-issue-reference.md`.
